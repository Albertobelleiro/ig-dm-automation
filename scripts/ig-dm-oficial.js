...
async function writeMessage(input, text) {
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    // Method 1: Paste simulation (preserves \n in Lexical)
    try {
    ...
    try {
      ...
      for (let li = 0; li < lines.length; li++) {
        if (lines[li].length > 0) document.execCommand('insertText', false, lines[li]);
        if (li < lines.length - 1) { document.execCommand('insertParagraph', false, null); await sleep(50); }
      }
      await sleep(200);
      if (input.textContent.trim().length > 0 && input.innerHTML.includes('<')) return true;
    } catch (e) {}
    ...
    try {
      ...
      for (let li = 0; li < lines.length; li++) {
        if (lines[li].length > 0) document.execCommand('insertText', false, lines[li]);
        if (li < lines.length - 1) { document.execCommand('insertLineBreak', false, null); await sleep(50); }
      }
      await sleep(200);
